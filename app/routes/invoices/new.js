...
export default Ember.Route.extend({
  ...
  afterModel(hash) {
    ...
    if (hash.prevInvoice.rejectBy("dirtyType", "created").get("lastObject")) {
      ...
      const prevInvoiceNumber = hash.prevInvoice
        .rejectBy("dirtyType", "created")
        .get("lastObject")
        .get("invoiceNumber");
      const numberToIncrement = prevInvoiceNumber.match(/\d+$/)[0];
      const increment = zeroPad(
        parseInt(numberToIncrement, 10) + 1,
        numberToIncrement.length
      );
      const invoiceNumber = prevInvoiceNumber.replace(
        numberToIncrement,
        increment
      );
  ...
  actions: {
    willTransition(transition) {
      if (transition.intent.name === "invoices") {
